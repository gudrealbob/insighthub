...
def parse_recommendation(text: str):
    """
    Very simple rule-based parser (Sprint 2 scope only).
    """
    ...
    # SYMBOL (very simple assumption: word after BUY/SELL)
    symbol_match = re.search(r"(BUY|SELL)\s+([A-Z]+)", text)
    symbol = symbol_match.group(2) if symbol_match else None

    # ENTRY (supports single number or range)
    ...
    if entry_match:
        ...
        entry_low = float(entry_single.group(1)) if entry_single else None
        entry_high = entry_low

    # STOP LOSS
    sl_match = re.search(r"(SL|STOP LOSS)\s*([0-9]+)", text)
    stop_loss = float(sl_match.group(2)) if sl_match else None

    # TARGETS
    targets = re.findall(r"TARGET\s*[0-9]*\s*([0-9]+)", text)
    ...
    # RISK (optional)
    risk_match = re.search(r"RISK\s*(LOW|MEDIUM|HIGH)", text)
